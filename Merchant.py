...
class Merchant:
    def __init__(self, id, PK, pgs_puk):
        self.id = id
        self.PK = PK
        self.PUK = dict()
        self.customers = dict()
    ...
    def give_ticket(self, customer_id, hidden=True):
        if hidden:  ##TODO: not hidden
            K = ""
            customer_addr = ""
            ...
            def send_auth2c():
                def gen_auth2c():
                    ...
                    m2c = crypto.encrypt_asym_with_signature(T_CM, self.PK[1], self.PUK[customer_id])
                    ##TODO: send T_CM, CM
                ...
                def parse_auth2m():
                    ...
                    customer_addr = ""  ## TODO
                    customer_id = ""  ##TODO
                    c2m, valid = crypto.decrypt_asym_with_signature(message, self.PK[1], self.PUK[customer_id])
                    if not valid:
            ...
            def m2c():
                ...
                ts = dt.now().timestamp()
                lf = 0.1  ##TODO
                T_CM = b''.join([ps, customer_addr, self.id, ts, lf])
                self.CM[ps] = (T_CM, CM)
                m2c = crypto.encrypt_asym_with_signature(T_CM, self.PK[1], self.PUK[ps])
                ##TODO: send T_CM, CM to the customer
